...
def generate_input(
    ...
    a_ref = torch.randint(
        0, 4, (B, m, k // 2), dtype=torch.uint8, device="cuda"
    ).permute(1, 2, 0)  #                        [m, k, B]
    # Pad b tensor's N dimension to 128 to call torch._scaled_mm for nvfp4 dot product computation
    b_ref = torch.randint(
        0, 4, (B, n_padded_128, k // 2), dtype=torch.uint8, device="cuda"
    ).permute(1, 2, 0) # 这里 m 维度只有 1 维有用?  [128, k, B]
    a_ref = a_ref.view(torch.float4_e2m1fn_x2)
    b_ref = b_ref.view(torch.float4_e2m1fn_x2)

    # Create float16 output tensor
    c_ref = torch.randn((B, m, n), dtype=torch.float16, device="cuda").permute(
        1, 2, 0
    ) # [m, n, B]
    ...
    sf_k = ceil_div(k, sf_vec_size) # 16个元素一个 sf_k
    sfa_ref_cpu, sfa_permuted = create_scale_factor_tensors(B, m, sf_k)
    sfb_ref_cpu, sfb_permuted = create_scale_factor_tensors(B, n_padded_128, sf_k)
